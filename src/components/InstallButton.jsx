...
// iOS에서 Safari가 아닌 인앱 브라우저(카톡/인스타/밴드/크롬 등) 판별.
// 인앱 브라우저에서는 "홈 화면에 추가"가 불가능하므로 Safari로 유도해야 함.
function isIOSInAppBrowser() {
  if (typeof navigator === 'undefined') return false;
  const ua = navigator.userAgent || '';
  if (!/iPhone|iPad|iPod/.test(ua)) return false;
  return /KAKAOTALK|Instagram|FBAN|FBAV|NAVER|Line\/|BAND|crios|fxios|edgios|whale/i.test(ua);
}

function openInSafari() {
  // x-safari-https:// 스킴: iOS 15+ 에서 현재 URL을 Safari로 전달.
...
function InstallModal({ platform, onClose }) {
  ...
  const inApp = isIOS && isIOSInAppBrowser();
  const steps = isIOS
    ? [
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div style={{ fontSize: '0.8125rem', color: 'var(--color-gray-500)', marginBottom: 14 }}>
          {isIOS
            ? (inApp
                ? '지금은 카톡·인스타 등 인앱 브라우저예요. 먼저 Safari로 열어야 설치할 수 있어요.'
          ...
          <button
            ...
            style={{
              ...
              background: inApp
                ...
                : 'white',
              color: inApp ? 'white' : 'var(--color-pink)',
              border: inApp ? 'none' : '1.5px solid var(--color-pink)',
              fontSize: '0.9375rem', fontWeight: 800,
              display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8,
              minHeight: 52,
              boxShadow: inApp ? '0 4px 12px rgba(255, 75, 110, 0.3)' : 'none',
            }}
          >
            <span style={{ fontSize: '1.125rem' }}>🧭</span>
