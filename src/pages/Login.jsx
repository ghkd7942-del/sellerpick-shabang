...
export default function Login() {
  ...
  // 고객 카카오 로그인
  const handleCustomerKakao = async () => {
    setError('');
    setLoading(true);
    try {
      // Firebase Kakao OAuth 미구성 시 temp 세션으로 진입
  ...
  // 관리자 로그인 — Google OAuth로 진행 후 이메일 허용 목록 확인
  const handleAdminLogin = async () => {
    ...
    try {
      ...
      if (isAdmin({ email })) {
        ...
        setError(
          `관리자 권한이 없는 계정이에요 (${email}).\n등록된 관리자: ${ADMIN_EMAILS.join(', ')}`
        );
      }
    } catch (err) {
      if (err.code !== 'auth/popup-closed-by-user') {
        setError('관리자 로그인 실패: ' + (err.message || '다시 시도해주세요'));
      }
    }
    setLoading(false);
  };
    ...
    <div style={{
      ...
      <div style={{
        ...
        <p style={{
          ...
          color: 'var(--color-gray-500)', marginTop: 20, lineHeight: 1.6,
        }}>
